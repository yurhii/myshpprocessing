...
require('vertex.inc.php');		// A polygon consists of vertices. So the polygon
...
*/
	...
	function isInside (&$v)
	{
		...
		do
		{
			if (!$q->isIntersect())
			{
				...
				if($this->isPartOfLine($v, $q, $this->nxt($q->Next()))){
					$partOfLine = true;
				}
			}
			$q = $q->Next();
		...
		if ($winding_number % 2 == 1 || $partOfLine){	// Check even or odd
			return true;				// odd == inside
		}else{
			return false;
		}	
	}
	//ver si v esta en la linea formada por p y q
	function isPartOfLine(&$v, &$p1, &$p2){
		$x1 = $v->X();
		$y1 = $v->Y(); 
		$x2 = $p1->X(); 
		$y2 = $p1->Y(); 
		...
		if($x3 != $x2 || $y3 != $y2){ 
			if($x3 != $x2){
				$m = ($y3-$y2)/($x3-$x2);
				$b = $y2 - $m*$x2;
				if( ($m*$x1+ $b - $y1) == 0 && $x1 <= max($x2, $x3) && $x1 >= min($x2, $x3) && $y1 >= min($y2, $y3) && $y1 <= max($y2, $y3)){
					$inLine = true;
				}else{
					$inLine = false;
				}
				...
				$m = 0;
				$b = $x2;
				if($b == $x1 && $y1<= max($y2, $y3) && $y1 >= min($y2, $y3)){
					$inLine = true;
				}else{
					$inLine = false;
				}
	...
	*/
	...
	function isPolyInsideSimple (&$p)
	{
		$inside = TRUE;
		$c =& $p->getFirst();	// Get the first vertex in polygon $p
		do
		{
			if (!$this->isInsideBox($c)) // If vertex is NOT inside this polygon
				$inside = FALSE;		// then set flag to false
			$c =& $c->Next();			// Get the next vertex in polygon $p
		}
		while ($c->id() != $p->first->id());
	...
	*/
	...
	function bRectArray(){
		...
		while($v->id() != $this->first->id());
		$bRect = array($minX, $maxX, $minY, $maxY);
		return $bRect;
	}
	function intersectPolyBox($poly){
		$b1 = $this->bRectArray();
		$b2 = $poly->bRectArray();
		if( ( ($b1[0] <= $b2[0]  && $b1[1] >= $b2[0]) || ($b1[0] > $b2[0] && $b1[0] < $b2[1]) ) && ( ($b1[2] <= $b2[2]  && $b1[3] >= $b2[2]) || ($b1[2] > $b2[2] && $b1[2] < $b2[3]) ) ){
			return true;
		}else{
			return false;
		}
	...
	*/
	...
	function intersectPoly($q){
		$chico = ($this->width > $q->width) ? $q : $this;
		$grande = ($this->width > $q->width) ? $this : $q;
		$poly = $grande->boolean($chico, 'A&B');
		if($grande->isEqualTo($poly)){
			//echo "pligono iguales<br />";
			if($chico->isPolyInsideSimple($grande) || $grande->isPolyInsideSimple($chico)){
				//echo "se intersectan<br />";
				return true;
			}else{
				return false;
			}
	...
	function isEqualTo($q){
		$v1 = $this->getFirst();
		$v2 = $q->getFirst();
		$cuenta = 0;
		while($cuenta < $this->cnt && $v1->X() == $v2->X() && $v1->Y() == $v2->Y()){
			$v1 = $v1->Next();
			$v2 = $v2->Next();
			$cuenta++;
		}
	...
	function setColour($string, $image) {
	...
	function drawPolyAt($x, $y, $i, $color)
	{
	  $p = $this;
	  $colour = $this->setColour($color, $i->image);
	  if ($i->image) $sy = imagesy($i->image); // Determine the height of the image in pixels
		...
		do                       // For all polygons in the list
		{
		  ...
		  do                     // For all vertices in this polygon
		  {
			...
			{                    // It is a line
			  ...
			  imageline($i->image,$x+$v->X(),$i->getHeight() -($sy-($y+$v->Y())),$x+$n->X(), $i->getHeight() - ($sy-($y+$n->Y())),$colour);
			}
			else
			{                    // It is an Arc
			  ...
			  if ($v->d() == -1) // Clockwise
				imagearc($i->image, $x+$v->Xc(), $sy-($y+$v->Yc()),$dia,$dia,$s,$e,$colour);
			  else               // Anti-Clockwise
				imagearc($i->image, $x+$v->Xc(), $sy-($y+$v->Yc()),$dia,$dia,$e,$s,$colour);
			}
